...
class Merlin {
    ...
    */
    public function admin_page() {
        ...
        $texts = array(
            'something_went_wrong' => esc_html__( 'Something went wrong. Please refresh the page and try again!', 'venam' ),
        );
    ...
    */
    public function svg( $args = array() ) {
        ...
        if ( empty( $args ) ) {
            return esc_html__( 'Please define default parameters in the form of an array.', 'venam' );
        }

        // Define an icon.
        if ( false === array_key_exists( 'icon', $args ) ) {
            return esc_html__( 'Please define an SVG icon filename.', 'venam' );
        }
    ...
    public function steps() {
        ...
        if ( ! empty( $this->import_files ) ) {
            $this->steps['content'] = array(
                'name' => esc_html__( 'Content', 'venam' ),
                'view' => array( $this, 'content' ),
            );
        }
        $this->steps['ready'] = array(
            'name' => esc_html__( 'Ready', 'venam' ),
    ...
    protected function server_requirements($step) {
        ...
        <table class="widefat" cellspacing="0">
            <tbody>
                ...
                <?php if ( function_exists( 'ini_get' ) ) : ?>
                    ...
                    if ( function_exists( 'phpversion' ) ) {
                        ...
                        <?php if ($php_check == 'error'): ?>
                            <tr>
                                <td><?php esc_html_e( 'PHP Version', 'venam' ); ?></td>
                                <td>
                                    <span class="dashicons dashicons-warning red"></span> <span><?php echo esc_html($phpversion); ?></span>
                                    <div><small><?php esc_html_e('Recommended: ', 'venam'); ?><strong>7.0 or higher</strong></small></div>
                                </td>
                            </tr>
                        <?php endif; ?>
                    <?php	} ?>
                    ...
                    <?php if ($max_input_vars == 'error'):
                        ...
                        ?>
                        <tr>
                            <td><?php esc_html_e( 'max_input_vars', 'venam' ); ?></td>
                            <td>
                                <span class="dashicons dashicons-warning <?php echo esc_attr($color); ?>"></span> <span><?php echo ini_get('max_input_vars'); ?></span>
                                <div><small><?php esc_html_e('Recommended: ', 'venam'); ?><strong>3000</strong></small></div>
                            </td>
                        </tr>
                    <?php endif; ?>
                    ...
                    <?php if ($memory_limit == 'error'): ?>
                        <tr>
                            <td><?php esc_html_e( 'memory_limit', 'venam' ); ?></td>
                            <td>
                                <span class="dashicons dashicons-warning red"></span> <span><?php echo ini_get('memory_limit'); ?></span>
                                <div><small><?php esc_html_e('Recommended: ', 'venam'); ?><strong>256M</strong></small></div>
                            </td>
                        </tr>
                    <?php endif; ?>
                    ...
                    <?php if ($max_execution_time == 'error'): ?>
                        <tr>
                            <td><?php esc_html_e( 'max_execution_time', 'venam' ); ?></td>
                            <td>
                                <span class="dashicons dashicons-warning red"></span> <span><?php echo ini_get('max_execution_time'); ?></span>
                                <div><small><?php esc_html_e('Recommended: ', 'venam'); ?><strong>300</strong></small></div>
                            </td>
                        </tr>
                    <?php endif; ?>
                    ...
                    <?php if ($upload_max_filesize == 'error'): ?>
                        <tr>
                            <td data-export-label="Max Upload Size"><?php esc_html_e( 'Max Upload Size', 'venam' ); ?>:</td>
                            <td>
                                <span class="dashicons dashicons-warning red"></span> <span><?php echo size_format( wp_max_upload_size() ); ?></span>
                                <div><small><?php esc_html_e('Recommended: ', 'venam'); ?><strong>12M</strong></small></div>
                            </td>
                        </tr>
                    <?php endif; ?>
                ...
                $posting['fsockopen_curl']['help'] = '<a href="#" class="help_tip" data-tip="' . esc_attr__( 'Payment gateways can use cURL to communicate with remote servers to authorize payments, other plugins may also use it when communicating with remote services.', 'venam' ) . '">[?]</a>';
        ...
        <?php if ( $go == false) { ?>
            <?php if ( $step == 'welcome' ) : ?>
                <p class="server_check_notice step_welcome red"><?php echo sprintf( __( 'Your server does not match the recommended settings. You might not be able to import the full demos.', 'venam' ), '<span class="underline">', '</span>' ); ?></p>
                <p class="server_check_notice step_welcome"><?php echo sprintf( __( '%sAdjust your settings%s, or proceed with caution :(', 'venam' ), '<a href="https://ninetheme.com/docs/wordpress-server-requirements/" target="_blank">', '</a>' ); ?></p>
            <?php else: ?>
                <p class="server_check_notice step_import red"><?php echo sprintf( __( 'Your server does not match the recommended settings. %sImport the BASIC Demo%s only, or adjust your settings.', 'venam' ), '<span class="underline">', '</span>' ); ?></p>
            <?php endif; ?>
        <?php } ?>
    ...
    */
    protected function plugins() {
        ...
        if(!$purchase_code) {
            echo '<p>' . esc_html__('Please verify the purchase code first.', 'venam') . '</p>';
                echo sprintf( '<a class="relative" href="%s">%s</a>',
                esc_url( admin_url( '/' ) ),
                esc_html__('Back Admin Page', 'venam')
                );
            exit(0);
        }
        // Variables.
                        ...
                        <?php foreach ( $required_plugins as $slug => $plugin ) : ?>
                            <li data-slug="<?php echo esc_attr( $slug ); ?>">
                                ...
                                <label for="default_plugins_<?php echo esc_attr( $slug ); ?>">
                                    ...
                                    <span class="badge">
                                        <span class="hint--top" aria-label="<?php esc_attr_e( 'Required', 'venam' ); ?>">
                                            <?php esc_html_e( 'req', 'venam' ); ?>
                                        </span>
                                    </span>
                                </label>
                            </li>
    ...
    */
    protected function content() {
        ...
        $landing_url = function_exists( 'venam_merlin_local_import_files' ) ? venam_merlin_local_import_files() : array();
            ...
            <?php if ( 1 < count( $this->import_files ) ) : ?>
                ...
                <div class="merlin__select-control-wrapper">
                    ...
                    <div class="merlin__select-control-help">
                        <span class="hint--top" aria-label="<?php echo esc_attr__( 'Select Demo', 'venam' ); ?>">
                    ...
                    <div class="merlin__select-control-preview">
                        ...
                        <?php if ( !empty( $this->import_files[0]['import_preview_url'] ) ) : ?>
                            <a class="current-demo" href="<?php echo esc_url( $this->import_files[0]['import_preview_url'] ); ?>" target="_blank"><?php esc_html_e( 'See ', 'wp-merlin' ); ?><span><?php echo esc_html( $this->import_files[0]['import_file_name'] ); ?></span></a>
                        <?php endif; ?>
                    </div>
                </div>
    ...
    public function _ajax_activate_license() {
        ...
        if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) ) {
            wp_send_json(
                array(
                    'success' => false,
                    'message' => esc_html__( 'Yikes! The theme activation failed. Please try again or contact support.', 'venam' ),
                )
            );
        }
        ...
        if ( empty( $license_key ) ) {
            wp_send_json(
                array(
                    'success' => false,
                    'message' => esc_html__( 'Please add your license key before attempting to activate one.', 'venam' ),
                )
            );
        }
        ...
        switch ($output->status) {
            case 200:
                add_option('envato_purchase_code_33546846', $license_key, '', 'yes');
                if ( 'yes' == get_option('ninetheme_deactivated_licence') ) {

                    delete_option( 'ninetheme_deactivated_licence' );
                    $href = esc_url( admin_url('admin.php?page=ninetheme_about_page') );
                    $btn = esc_html__( 'Return to the Dashboard', 'venam' );
                    $title = esc_html__( 'Activated', 'venam' );
                    $wait = esc_html__( 'The activation is done!', 'venam' );

                    wp_send_json(array('success' => true,'complete' => true, 'href' => $href,'wait' => $wait,'title' => $title,'message' => esc_html__($output->msg, 'venam' )));

                } else {
                    wp_send_json(array('success' => true, 'message' => esc_html__($output->msg, 'venam' )));
                }

            break;
            case 400:
                wp_send_json(array('success' => false, 'message' => esc_html__($output->msg, 'venam' )));
            break;
            case 404:
                wp_send_json(array('success' => false, 'message' => esc_html__($output->msg, 'venam' )));
            break;
    ...
    protected function edd_activate_license( $license ) {
        ...
        if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
            ...
            if ( is_wp_error( $response ) ) {
                ...
                $message = esc_html__( 'An error occurred, please try again.', 'venam' );
            }
        } else {
            ...
            if ( false === $license_data->success ) {
                ...
                switch ( $license_data->error ) {
                    ...
                    $message = sprintf(
                        ...
                        esc_html__( 'Your license key expired on %s.', 'venam' ),
                        date_i18n( get_option( 'date_format' ), strtotime( $license_data->expires, current_time( 'timestamp' ) ) )
                    );
                    break;

                    case 'revoked':
                    $message = esc_html__( 'Your license key has been disabled.', 'venam' );
                    break;

                    case 'missing':
                    $message = esc_html__( 'This appears to be an invalid license key. Please try again or contact support.', 'venam' );
                    break;

                    case 'invalid':
                    case 'site_inactive':
                    $message = esc_html__( 'Your license is not active for this URL.', 'venam' );
                    break;

                    case 'item_name_mismatch':
                    /* translators: EDD Item Name */
                    $message = sprintf( esc_html__( 'This appears to be an invalid license key for %s.', 'venam' ), $this->edd_item_name );
                    break;

                    case 'no_activations_left':
                    $message = esc_html__( 'Your license key has reached its activation limit.', 'venam' );
                    break;
...
";
    ...
    */
    function _ajax_plugins() {
        ...
        foreach ( $plugins['activate'] as $slug => $plugin ) {
            if ( $_POST['slug'] === $slug ) {
                $json = array(
                    ...
                    'action2' => - 1,
                    'message' => esc_html__( 'Activating', 'venam' ),
                );
                break;
            }
        }
        ...
        foreach ( $plugins['update'] as $slug => $plugin ) {
            if ( $_POST['slug'] === $slug ) {
                $json = array(
                    ...
                    'action2' => - 1,
                    'message' => esc_html__( 'Updating', 'venam' ),
                );
                break;
            }
        }
        ...
        foreach ( $plugins['install'] as $slug => $plugin ) {
            if ( $_POST['slug'] === $slug ) {
                $json = array(
                    ...
                    'action2' => - 1,
                    'message' => esc_html__( 'Installing', 'venam' ),
                );
                break;
            }
        }
        ...
        if ( $json ) {
            ...
            $json['message'] = esc_html__( 'Installing', 'venam' );
            wp_send_json( $json );
            ...
            wp_send_json(
                array(
                    'done' => 1,
                    'message' => esc_html__( 'Success', 'venam' ),
                )
            );
        }
    ...
    function _ajax_content() {
        ...
        if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) || empty( $_POST['content'] ) && isset( $content[ $_POST['content'] ] ) ) {
            ...
            wp_send_json_error(
                array(
                    'error' => 1,
                    'message' => esc_html__( 'Invalid content!', 'venam' ),
                )
            );
        }
        ...
        if ( $json ) {
            ...
            wp_send_json(
                array(
                    'error' => 1,
                    'message' => esc_html__( 'Error', 'venam' ),
                    'logs' => '',
                    'errors' => '',
                )
            );
    ...
    public function _ajax_get_total_content_import_items() {
        if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) && empty( $_POST['selected_index'] ) ) {
            ...
            wp_send_json_error(
                array(
                    'error' => 1,
                    'message' => esc_html__( 'Invalid data!', 'venam' ),
                )
            );
        }
    ...
    protected function get_import_data( $selected_import_index = 0 ) {
        ...
        if ( ! empty( $import_files['content'] ) ) {
            $content['content'] = array(
                'title' => esc_html__( 'Content', 'venam' ),
                'description' => esc_html__( 'Demo content data.', 'venam' ),
                'pending' => esc_html__( 'Pending', 'venam' ),
                'installing' => esc_html__( 'Installing', 'venam' ),
                'success' => esc_html__( 'Success', 'venam' ),
        ...
        if ( ! empty( $import_files['widgets'] ) ) {
            $content['widgets'] = array(
                'title' => esc_html__( 'Widgets', 'venam' ),
                'description' => esc_html__( 'Sample widgets data.', 'venam' ),
                'pending' => esc_html__( 'Pending', 'venam' ),
                'installing' => esc_html__( 'Installing', 'venam' ),
                'success' => esc_html__( 'Success', 'venam' ),
        ...
        if ( ! empty( $import_files['sliders'] ) ) {
            $content['sliders'] = array(
                'title' => esc_html__( 'Revolution Slider', 'venam' ),
                'description' => esc_html__( 'Sample Revolution sliders data.', 'venam' ),
                'pending' => esc_html__( 'Pending', 'venam' ),
                'installing' => esc_html__( 'Installing', 'venam' ),
                'success' => esc_html__( 'Success', 'venam' ),
        ...
        if ( ! empty( $import_files['options'] ) ) {
            $content['options'] = array(
                'title' => esc_html__( 'Options', 'venam' ),
                'description' => esc_html__( 'Sample theme options data.', 'venam' ),
                'pending' => esc_html__( 'Pending', 'venam' ),
                'installing' => esc_html__( 'Installing', 'venam' ),
                'success' => esc_html__( 'Success', 'venam' ),
        ...
        if ( ! empty( $import_files['redux'] ) ) {
            $content['redux'] = array(
                'title' => esc_html__( 'Redux Options', 'venam' ),
                'description' => esc_html__( 'Redux framework options.', 'venam' ),
                'pending' => esc_html__( 'Pending', 'venam' ),
                'installing' => esc_html__( 'Installing', 'venam' ),
                'success' => esc_html__( 'Success', 'venam' ),
        ...
        if ( ! empty( $import_files['option_tree'] ) ) {
            $content['option_tree'] = array(
                'title' => esc_html__( 'Option Tree Options', 'venam' ),
                'description' => esc_html__( 'Option Tree framework options.', 'venam' ),
                'pending' => esc_html__( 'Pending', 'venam' ),
                'installing' => esc_html__( 'Installing', 'venam' ),
                'success' => esc_html__( 'Success', 'venam' ),
        ...
        if ( ! empty( $import_files['cptui'] ) ) {
            $content['cptui'] = array(
                'title' => esc_html__( 'CPTUI Options', 'venam' ),
                'description' => esc_html__( 'CPTUI framework options.', 'venam' ),
                'pending' => esc_html__( 'Pending', 'venam' ),
                'installing' => esc_html__( 'Installing', 'venam' ),
                'success' => esc_html__( 'Success', 'venam' ),
        ...
        if ( false !== has_action( 'merlin_after_all_import' ) ) {
            $content['after_import'] = array(
                'title' => esc_html__( 'After import setup', 'venam' ),
                'description' => esc_html__( 'After import setup.', 'venam' ),
                'pending' => esc_html__( 'Pending', 'venam' ),
                'installing' => esc_html__( 'Installing', 'venam' ),
                'success' => esc_html__( 'Success', 'venam' ),
    ...
    */
    public function pt_importer_new_ajax_request_response_data( $data ) {
        ...
        $data['message']  = esc_html__( 'Installing', 'venam' );
        $data['proceed']  = 'true';
