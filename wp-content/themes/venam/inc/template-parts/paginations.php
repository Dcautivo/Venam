...
if ( ! function_exists( 'venam_posts_prev_pag_attrs' ) ) {
    function venam_posts_next_pag_attrs()
    {
        return 'class="nt-pagination-link -next"';
    }
    add_filter( 'next_posts_link_attributes', 'venam_posts_next_pag_attrs' );
}
...
if ( ! function_exists( 'venam_posts_prev_pag_attrs' ) ) {
    function venam_posts_prev_pag_attrs()
    {
        return 'class="nt-pagination-link -previous"';
    }
    add_filter( 'previous_posts_link_attributes', 'venam_posts_prev_pag_attrs' );
}
...
if ( ! function_exists( 'venam_single_navigation' ) ) {
    function venam_single_navigation()
    {
        if ( '0' != venam_settings( 'single_navigation_visibility', '1' ) ) {
            ...
            <div class="pagination">
                <?php 
                if ( $prev ) {
                    previous_post_link( '%link', _x('PREVIOUS POST', 'Previous post link', 'venam' ) );
                } else {
                ...
                <?php 
                if ( $next ) {
                    next_post_link( '%link', _x('NEXT POST', 'Next post link', 'venam' ) );
                }
                ?>
            </div>
            <?php
...
if ( ! function_exists( 'venam_single_navigation_two' ) ) {
    function venam_single_navigation_two()
    {
        if ( '1' == venam_settings( 'single_navigation_visibility', '0' ) ) {
...
if ( ! function_exists( 'venam_index_loop_pagination' ) ) {
    //add_action('venam_loop_pagination', 'venam_index_loop_pagination' );
    function venam_index_loop_pagination()
    {
        if ( '0' !=  venam_settings( 'pagination_visibility', '1' ) ) {
            $pag_class= array();
            $pag_class[] = ('yes' == venam_settings( 'pag_group') ) ? '-group' : '';
            $pag_class[] = '-style-'.venam_settings( 'pag_type', 'default' );
            $pag_class[] = '-size-'.venam_settings( 'pag_size', 'medium' );
            $pag_class[] = '-corner-'.venam_settings( 'pag_corner', 'square' );
            $pag_class[] = '-align-'.venam_settings( 'pag_align', 'center' );
...
if ( ! function_exists( 'venam_current_link_pages' ) ) {
    function venam_current_link_pages( $link )
    {
        if ( ctype_digit( $link ) ) {
    ...
    add_filter( 'wp_link_pages_link', 'venam_current_link_pages' );
}
...
if ( ! function_exists( 'venam_wp_link_pages' ) ) {
    function venam_wp_link_pages()
    {
        // pagination for page links
        wp_link_pages( array(
            'before' => '<div class="clearfix"></div><div class="page-links"><span class="page-links-title">' . esc_html__( 'Pages', 'venam' ) . '</span>',
            'after' => '</div>',
            'link_before' => '',
            'link_after' => '',
            'next_or_number' => 'number',
            'separator' => ' ',
            'nextpagelink' => esc_html__( 'Next page', 'venam' ),
            'previouspagelink' => esc_html__( 'Previous page', 'venam' ),
            'pagelink' => '%',
            'echo' => 1
        ));
    }
