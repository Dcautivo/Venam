...
* @since 1.0.0
*/
function venam_string_shortcode() {

    return __( 'Shortcode', 'naturall' );

}  // end function

add_action( 'manage_' . Source_Local::CPT . '_posts_columns', 'venam_admin_columns_headers_elementor_templates' );
/**
* Add post type list table column for "Saved Templates" post type.
*
* @since 1.0.0
*
* @uses venam_string_shortcode()
*
* @param array $columns Holds all post type list table columns.
* @return array Modified array of post type list table columns.
*/
function venam_admin_columns_headers_elementor_templates( $columns ) {

    $columns[ 'shortcode' ] = venam_string_shortcode();
...
add_action( 'manage_' . Source_Local::CPT . '_posts_custom_column', 'venam_admin_columns_content_elementor_templates', 10, 2 );
/**
...
*/
function venam_admin_columns_content_elementor_templates( $column_name, $post_id ) {
    ...
    if ( 'shortcode' === $column_name ) {
        ...
        printf(
            '<input class="venam-shortcode-input widefat" type="text" readonly onfocus="this.select()" value="%s" />',
            $shortcode
        );
    }  // end if
}  // end function
...
if ( ! function_exists( 'venam_shortcode_elementor_template' ) ) {

    add_shortcode( 'venam-template', 'venam_shortcode_elementor_template' );
    /**
    ...
    */
    function venam_shortcode_elementor_template( $atts = [] ) {

        /** Set default shortcode attributes */
        $defaults = apply_filters(
            'venam/filter/shortcode_defaults/elementor_template',
            array(
                'id' => '',
                'css' => 'false',
            )
        ...
        $atts = shortcode_atts( $defaults, $atts, 'venam-template' );
        ...
        return apply_filters(
            'venam/filter/shortcode/elementor_template',
            $output,
