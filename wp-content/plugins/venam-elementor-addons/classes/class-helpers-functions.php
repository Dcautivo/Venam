...
trait Venam_Helper
{
    ...
    *
    */
    protected function venam_query_controls( $def="post", $pag = false, $filter = false )
    {
        $post_types = $this->venam_get_post_types();
        ...
        if ( true == $filter ) {
            foreach ( $post_types as $slug => $label ) {
                ...
                $this->add_control( $slug . '_filter_top_type_heading',
                    array(
                        ...
                        'label'       => sprintf( '%s %s', strtoupper( $label ), __( 'TOP FILTER', 'venam' ) ),
                ...
                if ( ! empty( $taxonomy ) ) {
                    ...
                    if ( ! empty( $tax_terms ) ) {
                        ...
                        $this->add_control( $slug . '_top_taxonomy',
                            array(
                                /* translators: %s Label */
                                'label' => sprintf( __( '%s Taxonomy Type', 'venam' ), $label ),
            ...
            foreach ( $post_types as $slug => $label ) {
                ...
                if ( ! empty( $taxterms ) ) {
                    $this->add_control( $slug . '_top_filter',
                        array(
                            /* translators: %s Label */
                            'label' => sprintf( __( '%s Taxonomy Exclude', 'venam' ), $label ),
                            'type' => Controls_Manager::SELECT2,
                            'default' => '',
                            'multiple' => true,
                            'label_block' => true,
            ...
            foreach ( $post_types as $slug => $label ) {
                ...
                $this->add_control( $slug . '_filter_type_heading',
                    array(
                        /* translators: %s: post type label */
                        'label'       => sprintf( __( '%s FILTER', 'venam'), strtoupper( $label ) ),
                        'type'        => Controls_Manager::HEADING,
                        'separator'   => 'before',
                        'condition' => [ 'post_type' => $slug ]
                    )
                    ...
                    foreach ( $taxonomy as $index => $tax ) {
                        ...
                        if ( ! empty( $terms ) ) {
                            ...
                            $this->add_control( $index . '_' . $slug . '_filter_type',
                                [
                                    'label' => sprintf( __( '%s Filter Type', 'venam' ), $tax->label ),
                                    'type' => Controls_Manager::SELECT,
                                    'default' => 'IN',
                                    'label_block' => true,
                                    'options' => [
                                        'IN' => sprintf( __( 'Include %s', 'venam' ), $tax->label ),
                                        'NOT IN' => sprintf( __( 'Exclude %s', 'venam' ), $tax->label ),
                                    ],
                                    'condition' => [ 'post_type'  => $slug ]
                                ]
                            );
        ...
        $this->add_control( 'author_filter_type',
            array(
                'label' => esc_html__( 'Authors Filter Type', 'venam' ),
                'type' => Controls_Manager::SELECT,
                'default' => 'author__in',
                'label_block' => true,
                'separator' => 'before',
                'options' => array(
                    'author__in' => esc_html__( 'Include Authors', 'venam' ),
                    'author__not_in' => esc_html__( 'Exclude Authors', 'venam' ),
                ),
            )
        );
        $this->add_control( 'author',
            [
                'label' => esc_html__( 'Author', 'venam' ),
                'type' => Controls_Manager::SELECT2,
                'multiple' => true,
                'options' => $this->venam_get_users(),
        ...
        foreach ( $post_types as $slug => $label ) {
            ...
            $this->add_control( $slug . '_filter_type',
                array(
                    /* translators: %s: post type label */
                    'label'       => sprintf( __( '%s Filter Type', 'venam' ), $label ),
                    'type'        => Controls_Manager::SELECT,
                    ...
                    'options'     => array(
                        /* translators: %s: post type label */
                        'post__in'     => sprintf( __( 'Include %s', 'venam' ), $label ),
                        /* translators: %s: post type label */
                        'post__not_in' => sprintf( __( 'Exclude %s', 'venam' ), $label ),
                    ),
                    'condition' => [ 'post_type' => $slug ]
                )
            );
        ...
        $this->add_control( 'orderby',
            [
                ...
                'options' => [
                    ...
                    'author' => esc_html__( 'Author', 'venam' ),
                    'title' => esc_html__( 'Title', 'venam' ),
                    'name' => esc_html__( 'Slug', 'venam' ),
                    'date' => esc_html__( 'Date', 'venam' ),
                    'modified' => esc_html__( 'Last Modified Date', 'venam' ),
                    'parent' => esc_html__( 'Post Parent ID', 'venam' ),
                    'rand' => esc_html__( 'Random', 'venam' ),
                    'comment_count' => esc_html__( 'Number of Comments', 'venam' ),
                ],
                'default' => 'none'
            ]
        );
    ...
    * @return array
    */
    public function venam_get_elementor_templates( $type = null )
    {
        $args = [
     ...
     * List Posts
     */
    public function venam_get_posts() {
    ...
    * List Blog Users
    */
    public function venam_get_users()
    {
        $users = get_users();
        $options = array();
     ...
     * List Categories
     */
    public function venam_get_categories()
    {
    ...
    * List Tags
    */
    public function venam_get_tags()
    {
        $tags = get_tags();
        $options = array();
    ...
    * @return array
    */
    public function venam_cpt_get_next_post_title() {
    ...
    * @return array
    */
    public function venam_cpt_get_next_post_permalink() {
        $next_post = get_next_post();
        if ( $next_post ) {
    ...
    * @return array
    */
    public function venam_cpt_get_prev_post_title() {
    ...
    * @return array
    */
    public function venam_cpt_get_prev_post_permalink() {
        $prev_post = get_previous_post();
        if ( $prev_post ) {
    ...
    * @return array
    */
    public function venam_get_post_types()
    {
        $types = array();
    ...
    * @return array
    */
    public function venam_cpt_taxonomies( $posttype, $value='id' )
    {
        $options = array();
    ...
    * @return array
    */
    public function venam_tribe_events_taxonomies($value='id')
    ...
    * @return array
    */
    public function venam_tribe_events_post()
    {
        global $post;
        $options = array();
    ...
    * @return array
    */
    public function venam_events_manager_post_ids()
    {
        $options = array();
    ...
    * @return array
    */
    public function venam_woo_attributes()
    {
        $options = array();
    ...
    * @return array
    */
    public function venam_woo_attributes_taxonomies()
    {
        $options = array();
        if ( class_exists( 'WooCommerce' ) ) {
    ...
    * @return array
    */
    public function venam_woo_get_skus()
    {
        $options = array();
        if ( class_exists( 'WooCommerce' ) ) {
            $args = array(
    ...
    */
    ...
    public function venam_registered_image_sizes() {
    ...
    // hex to rgb color
    public function venam_hextorgb($hex) {
        $hex = str_replace("#", "", $hex);
        if(strlen($hex) == 3) {
